...
exports.convertToCamoUrl = function(url) {
  ...
  return config.camo.url + '/' + hmac + '/' + new Buffer(url).toString('hex')
}
...
exports.buildSchemaValidator = function(path, fn, async) {
...
exports.verifyTeamspeakLinkRequest = function(id, digest) {
  ...
  var id = new Buffer(id, 'hex').toString()
...
exports.getXpNeededForLevel = function(level) {
  return config.xp.levelXpKey[level] ? config.xp.levelXpKey[level] : config.xp.levelXpKey.default
}