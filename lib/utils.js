...
exports.convertToCamoUrl = function(url) {
  ...
  var hmac = crypto.createHmac('sha1', config.camo.key)

  hmac.setEncoding('hex')
  hmac.write(url)
  hmac.end()

  return config.camo.url + '/' + hmac.read() + '/' + new Buffer(url).toString('hex')
}
...
exports.msToHumanReadable = function(ms) {
  ...
  _.each([
    ...
    if(val) {
      readable += val + ' ' + (val > 1 ? arr[2] : arr[1])

      if(index > 0 && index < all.length - 2) {
        readable += ', '
