...
exports.checkForLevelups = function checkForLevelups() {
  ...
  _.each(config.xp.levelXpKey, function(xp, level) {
      bulk.find({
        level: (level === 'default') ? { $gte: config.xp.rampUpDoneAtLevel } : _.parseInt(level),
        'xp.current': { $gte: xp }
      }).update({
        $inc: { level: 1 },
        $set: { 'xp.current': 0 }
...
var agenda = new Agenda().database(config.mongo.host + ':' + config.mongo.port + '/' + config.mongo.db)
