...
var HTTPError = exports.HTTPError = inherit(Error, {
  ...
  toHTMLString: function(nonce) {
    return template(errorPages.template)({
      title: (errorPages.content[this.status] && errorPages.content[this.status].title) || null,
      body: this.message ? this.message : (errorPages.content[this.status] ? errorPages.content[this.status].body : null),
      code: this.status + ' ' + http.STATUS_CODES[this.status],
      nonce: 'nonce="' + nonce + '"'
    })
  }
