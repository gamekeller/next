...
var Yodel = inherit(EventEmitter, {
  ...
  connect: function(config) {
    ...
    this.connection = dnode.connect(this.config.host, _.parseInt(this.config.port))

    _.each(['end', 'error'], _.bind(function(event) {
    ...
    this.connection.on('remote', _.bind(function(remote) {
      this.remote = remote
