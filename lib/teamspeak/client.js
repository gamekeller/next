...
var Client = inherit(EventEmitter, {
  ...
  connect: function(config) {
    ...
    this.connection.on('connect', function() {
      ...
      .then(this.send('servernotifyregister', { event: 'channel', id: 0 }))
      .then(function() {
        this.emit('connect')
        this.status = Client.STATUS_READY

        // Relay events
        each(['cliententerview', 'clientleftview', 'clientmoved', 'channeledited'], function(e) {
  ...
  getOnlineClients: function() {
    return this.send('clientlist', { '-uid': true, '-groups': true }).then(function(list) {
      return filter(isArray(list) ? list : [list], function(client) {
