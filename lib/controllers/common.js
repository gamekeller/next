...
router.use(function(req, res, next) {
  var allowedPaths = ['/privacy', '/account/delete', '/logout']

  if(req.user && !req.user.consent && !_.includes(allowedPaths, req.path))
    return res.returnWith('error', 'Um fortzufahren, stimme bitte der neuen Datenschutzerklärung zu.', '/privacy')
  else
    next()
})
...
router.post('/privacy', function(req, res) {
  ...
  req.user.save(function(err) {
    ...
    res.returnWith('success', 'Du hast der Datenschutzerklärung zugestimmt!', '/account')
  })
})
