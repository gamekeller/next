...
/**
 * POST /admin/user/:userid/edit/xp
 * Edit user XP
 */
router.post('/:userid/edit/xp', function(req, res, next) {
  var user = req.profile
  var data = req.body

  var levelVal = _.parseInt(data.levelVal)
  var xpVal = _.parseInt(data.xpVal)

  var doc = { $inc: {} }

  if(levelVal) {
    doc['$inc'].level = data.levelMode === 'sub' ? -(levelVal) : levelVal
  }

  if(xpVal) {
    doc['$inc']['xp.total'] = doc['$inc']['xp.current'] = data.xpMode === 'sub' ? -(xpVal) : xpVal
  }

  user.updateOne(doc, function(err) {
    if(err)
      return res.handleMongooseError(err, next)

    res.returnWith('success', req.app.config.admin.user.messages.successfullyUpdated, { username: user.username }, req.resolveUrl('../'))
  })
})
...
router.post('/:userid/delete', function(req, res, next) {
  req.profile.remove(function(err, user) {
    ...
    res.returnWith('success', 'Account gelöscht.', '/admin/user')
  })
})
